...
def staff(request):
    ...
    if not service_id:
        return _error('service_required', 'Bitte wählen Sie zuerst eine Behandlung.')
...
def availability(request):
    ...
    if not service_id or not staff_id or not day:
        return _error('invalid_request', 'Behandlung, Mitarbeiter und Datum sind erforderlich.')
...
def appointments(request):
    data = _json(request)
    required = ['service_id', 'staff_id', 'starts_at', 'first_name', 'last_name', 'email']
    if any(not str(data.get(key) or '').strip() for key in required):
        return _error('missing_fields', 'Bitte füllen Sie alle Pflichtfelder aus.')
    ...
    if '@' not in email:
        return _error('invalid_email', 'Bitte geben Sie eine gültige E-Mail-Adresse ein.')
    ...
    with transaction.atomic():
        ...
        if customer:
            ...
            customer.phone = str(data.get('phone') or '').strip()[:40]
    ...
    return JsonResponse({
        'ok': True,
        'created': created,
        'message': 'Ihr Termin wurde erfolgreich gespeichert.',
        'appointment': {
