...
def rpc(url, method, params, headers=None, timeout=120):
    ...
    request = urllib.request.Request(url, data=payload, headers={'Content-Type':'application/json', **(headers or {})}, method='POST')
    try:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        bookings_raw = rpc(ADMIN_URL,'getBookings',[{'date_from':'2000-01-01','date_to':'2035-12-31','booking_type':'all','order':'date_start_asc'}],headers=headers) or []

        raw_backup = {'exported_at':timezone.now().isoformat(),'company':company_info,'timezone':timezone_info,'services':services_raw,'providers':providers_raw,'clients':clients_raw,'bookings':bookings_raw,'work_calendars':{},'extra':{}}
        optional_calls = {
            'cancellation_policy':('getCancellationPolicy',[]), 'categories':('getCategoriesList',[False]),
            'locations':('getLocationsList',[False]), 'company_vacations':('getCompanyVacations',[]),
            'products':('getProductList',[]), 'feedbacks':('getFeedbacks',[]),
            'plugin_list':('getPluginList',[]), 'currency':('getCompanyCurrency',[]),
        }
        ...
        booking_dates = [parsed.date() for row in bookings_raw if isinstance(row,dict) and (parsed := parse_source_datetime(row.get('start_date'),source_tz))]
        ...
        for year,month in month_iter(schedule_start,schedule_end):
            try:
                raw_backup['work_calendars'][f'{year:04d}-{month:02d}'] = rpc(ADMIN_URL,'getWorkCalendar',[year,month,safe_int(provider_external_id,0)],headers=headers) or {}
            except Exception as exc:
        ...
        summary = {'source_clients':len(clients_raw),'source_bookings':len(bookings_raw),'source_services':len(services_raw),'source_providers':len(providers_raw),'customers_created':0,'customers_matched':0,'services_created':0,'services_updated':0,'appointments_created':0,'appointments_updated':0,'daily_schedules_written':0,'skipped_bookings':0}

        with transaction.atomic():
            archive_provider,_ = StaffMember.objects.get_or_create(display_name=f'A+ Esthetic (SimplyBook #{provider_external_id})',defaults={'role':'team','bio':'Importierter SimplyBook-Bestand. Nicht für neue Online-Buchungen verwenden.','active':True,'sort_order':9000})
            archive_provider.role='team'; archive_provider.active=True; archive_provider.sort_order=9000
            ...
            service_map={}; archive_services=[]
            for idx,row in enumerate(services_raw,start=1):
                if not isinstance(row,dict): continue
                external_id=str(row.get('id') or '').strip()
                if not external_id: continue
                service,created=Service.objects.update_or_create(slug=f'simplybook-event-{external_id}',defaults={
                    'name':str(row.get('name') or f'SimplyBook Service {external_id}').strip()[:140],
                    'description':str(row.get('description') or '')[:4000],
                    'duration_minutes':max(5,safe_int(row.get('duration'),30)),
                    'buffer_minutes':max(0,safe_int(row.get('buffertime_before'),0))+max(0,safe_int(row.get('buffertime_after'),0)),
                    'price_label':'','active':as_bool(row.get('is_active',True)),'bookable':False,'requires_confirmation':False,
                    'sort_order':5000+safe_int(row.get('position'),idx)})
                summary['services_created' if created else 'services_updated']+=1
                service_map[external_id]=service; archive_services.append(service)
            ...
            for row in clients_raw:
                ...
                if customer:
                    ...
                    if not customer.last_name and last_name: customer.last_name=last_name; changed.append('last_name')
                    if not customer.phone and phone: customer.phone=phone; changed.append('phone')
                    if changed: customer.save(update_fields=changed+['updated_at'])
                else:
            ...
            for row in bookings_raw:
                if isinstance(row,dict):
                    cid=str(row.get('client_id') or ''); counts[cid]=counts.get(cid,0)+1
            for row in bookings_raw:
                if not isinstance(row,dict): summary['skipped_bookings']+=1; continue
                booking_id=str(row.get('id') or '').strip(); event_id=str(row.get('event_id') or '').strip(); client_id=str(row.get('client_id') or '').strip()
                service=service_map.get(event_id); starts_at=parse_source_datetime(row.get('start_date'),source_tz); ends_at=parse_source_datetime(row.get('end_date'),source_tz)
                if not service or not starts_at or not booking_id: summary['skipped_bookings']+=1; continue
                if not ends_at or ends_at<=starts_at: ends_at=starts_at+timedelta(minutes=max(5,safe_int(row.get('event_duration'),service.duration_minutes)))
                ...
                for value in (row.get('text'),row.get('comment')):
                    ...
                    if value and value not in text_parts: text_parts.append(value)
                defaults={'customer':customer,'service':service,'staff':archive_provider,'starts_at':starts_at,'ends_at':ends_at,
                    'status':'confirmed' if as_bool(row.get('is_confirm')) else 'cancelled','source':'admin','notes_customer':'\n\n'.join(text_parts)[:3000],
                    'returning_customer':counts.get(client_id,0)>1,'marketing_opt_in':False,'cancellation_terms_accepted':False,'privacy_accepted':False}
                appointment,created=Appointment.objects.update_or_create(idempotency_key=f'simplybook-{booking_id}',defaults=defaults)
                ...
                if record_date: Appointment.objects.filter(pk=appointment.pk).update(created_at=record_date)

            for _month,month_data in raw_backup['work_calendars'].items():
                if not isinstance(month_data,dict) or '_unavailable' in month_data: continue
                for day_string,info in month_data.items():
                    if not isinstance(info,dict): continue
                    try: day=date.fromisoformat(str(day_string))
                    except ValueError: continue
                    closed=as_bool(info.get('is_day_off')); start_time=parse_clock(info.get('from')); end_time=parse_clock(info.get('to'))
                    if not closed and (not start_time or not end_time or end_time<=start_time): closed=True
                    DailyAvailabilityOverride.objects.update_or_create(staff=archive_provider,date=day,defaults={
                        'closed':closed,'start_time_1':None if closed else start_time,'end_time_1':None if closed else end_time,
                        'start_time_2':None,'end_time_2':None})
                    summary['daily_schedules_written']+=1
            if options.get('dry_run'): transaction.set_rollback(True)
        ...
        if summary['skipped_bookings']: self.stdout.write(self.style.WARNING(f"Skipped bookings: {summary['skipped_bookings']}"))
        self.stdout.write(self.style.SUCCESS('Dry run completed; all database writes rolled back.' if options.get('dry_run') else 'SimplyBook import completed. No customer emails were sent.'))
